...
const drivers = [
  ...
  { id: 4, name: "João", team: "McLaren" },
  { id: 5, name: "Pedro", team: "Ferrari" },
  { id: 6, name: "José", team: "Red Bull Racing" },
];
...
server.get("/drivers_by_team", async (request, response) => {
  const { team } = request.query as { team: string };
  const driversByTeam = drivers.filter((d) => d.team === team);
  response.type("application/json").code(200);
  return { drivers: driversByTeam };
});
